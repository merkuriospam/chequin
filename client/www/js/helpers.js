...
function renderTemplate (tmpl_name, tmpl_data, destino, metodo, ajx) {
    var metodo = typeof metodo !== 'undefined' ? metodo : 'html';
    var ajx = typeof ajx !== 'undefined' ? ajx : false;
    if (ajx) {
        var tmpl_dir = 'partials';
        ...
        $.ajax({
            ...
            success: function(data) {
                plom = _.template(data);
                switch(metodo) {
                    case 'append':
                        $(destino).append(plom(tmpl_data));
                        break;
                    default:
                        $(destino).html(plom(tmpl_data));
                }
            }
        });
    } else {
        plum = _.template($('#'+tmpl_name).html());
        switch(metodo) {
            case 'append':
                $(destino).append(plum(tmpl_data));
                break;
            default:
                $(destino).html(plum(tmpl_data));
        }
    }
    $(destino).animateCss('fadeInRight');
    return true;
}

function probar2() {
    papa = [{texto:'uno'},{texto:'dos'},{texto:'tres'}]
    app.renderTemplate('loop', papa, '#escenerario', 'append');
}

function debug(mensaje) {
    if (APP_DEBUG) console.log(mensaje);
}
